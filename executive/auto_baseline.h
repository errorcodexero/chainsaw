...
struct Auto_baseline_turn: public Executive_impl<Auto_baseline_turn>{

	#define AUTO_BASELINE_TURN_ITEMS(X)\
	X(Motion_profile,motion_profile)\
	X(Drivebase::Encoder_ticks,initial_encoders) \
	X(Countdown_timer,in_range) 

	STRUCT_MEMBERS(AUTO_BASELINE_TURN_ITEMS)

	Auto_baseline_turn(Drivebase::Encoder_ticks initial_encoders1):initial_encoders(initial_encoders1){}
	IMPL_STRUCT(Auto_baseline_turn,AUTO_BASELINE_TURN_ITEMS)

	Executive next_mode(Next_mode_info);
	Toplevel::Goal run(Run_info);
	bool operator==(Auto_baseline_turn const&)const;
};
